...
message_limit = 15
message_transnum = message_limit + 5
...
with TelegramClient('session_name', api_id, api_hash, proxy=proxy) as client:
    ...
    target_chat_ids = ['-1001194346770', '-1001031569976', '-1001291324441']
    chat_ids = [chat_id for chat_id in chat_ids if chat_id in target_chat_ids]# 从列表中移除该 chat_id
    time.sleep(6)
...
def process_webpage(url_base, message_limit):
    # 指定输出文件路径
    output_file_path = r'D:\hexoblog\source\telegram\telegram_economy.html'
...
""")  # 开始写入 HTML 模板
    ...
    for chat_id in chat_ids:
        ...
        download_folder = r'D:\hexoblog\source\telegram\economy'
        ...
        for div in top_limit_message_divs:
            ...
            for img in img_tags:
                ...
                if img_url and img_url.startswith('blob:'):
                    # 获取图片的文件名并添加后缀
                    new_img_url = 'economy/' + img_url.split('/')[-1] + ".jpg"
                ...
                for video in video_tags:  # 遍历每个视频
                    ...
                    if video_src and video_src.startswith('./progressive/document'):
                        ...
                        while retries < max_retries:
                            try:
                                # 设置下载路径和文件名
                                tab.set.download_path(r'D:\hexoblog\source\telegram\economy')  # 设置文件保存路径
                                tab.set.download_file_name(file_name)  # 设置重命名文件名
                                time.sleep(1)
                                download.click()
                                
                                # 修改视频标签属性
                                video_src = 'economy/' + file_name + ".mp4"
        ...
        directory_path = "D:/hexoblog/source/telegram/economy"
...
process_webpage(url_base, message_transnum)
